...
SQL - Structured Query Language의 축약어로 쿼리라고 보통 부릅니다. 구조화된 질의 언어라는 뜻으로 
  DB에게 특정한 데이터를 정리해서 가지고 오라고 명렬할 때 쓰는 언어.

  Query라는 용어 자체가 체계적인 방식으로 배열 또는 구성되어있다는 의미로, 명령어를 작성하는 데
  잇어서 큰 틀 안에서 작성하도록 짜여있다는 것인데, 이 틀을 잘 지키기만 한다면 DB 사용자가
...
키워드(예약어) - 특정 프로그래밍 언어 등에서 사전에 정의된 단어라는 의미로, 다른 곳에 중복해서
  ...
  다른 프로그래밍 언어보다도 쿼리문에서 예약어가 차지하는 비중이 상당히 크기 떄문에 키워드를 기준으로 
  SQL 학습을 하면 좀 효율적입니다.
...
3. DML(Data Manipulation Language) - 데이터를 조회하거나 '조작'하기 위한 언어
  - 주요 명렁어 : INSERT / SELECT / UPDATE / DELETE -> 얘네를 CreateReadUpdateDelete라고 합니다.

4. TCL(Transaction Control Language) - DB 트랜잭션을 제어하기 위한 언어
  - 주요 명렁어 : COMMIT / ROLLBACK / SAVEPOINT
  - 트랙잭션의 완료를 확정하거나 롤백을 작업에 사용
...
- SELECT ... FROM ~ (~ 테이블에서 ...을 가지고 와라)
  ...
  - SELECT (컬럼명<무조건인 건 아님.>) FROM(테이블명)
  - *(asterisk) : all의 의미
...
### 부가 구문(ORDER BY, LTMIT, CASE ... WHEN)
...
- 컬럼에서 정렬이 필요할 때
  ...
  - 하지만 테이블의 정의방식과 내부적으로 저장되는 시점, 그리고 데이터의 조작(입력, 수정, 삭제)이
    빈번하게 이루어진 테이블에서 데이터가 너무 많다면 SELECT 문을 실행할 때마다 데이터 순서가 바뀌는 경우가 있습니다.
    이상을 이유로 dafault 상으로 오름차순으로 정렬되었다고 하더라도 명시적으로 작성하지 않았다면
...
```sql
-- 상품주문 테이블에서
-- 회원아이디, 주문번호, 상품번호, 주문금액,                      
-- 주문수량, 결제금액, 컬럼을 조회하는데,                           
-- 결제 금액이 3000원 이상인 것들만 가지고 오고,                      
-- 회원 아이디 기준 오름차순, 상품 번호 기준으로 내림차순하는 쿼리문을
-- 작성하시오.
...
- 행의 개수를 조절해야할 때(다 가지고 오지말고 몇 개만 가지고 오라고 명령)
  ...
  있을 때 사용하는 명령어가 -> _LIMIT n_
    ...
    * 그런데 LIMIT는 다른 ROBS에서는 지원안하고, 보통 SQLD나 정보처리기사 문제에서는 _TOP(n)_의
    형태로 작성합니다.
...
- 조건에 따라서 값을 다르게 표현할 때(가져온 데이터가 ~ 이면 ~ 로 표현할 것) 
  ...
  낼 때가 있습니다. 이것은 그 예시라고 생각하시면 됩니다.
...
- 이상의 SQL문을 해석해보면 CASE WHEN ... THEN ... ELSE ... END의 패턴을 지니고 있는데, '()' 소괄호로
  묶어서 실제 회원 테이블에는 존재하지 않는 가상(Dummy) 컬럼을 만들어넀음을 확인할 수 있습니다.
  - 이상의 경우를 SQL에서는 표현식(Expression)이라고 말합니다.
  - 그리고 'AS'가 처음 나오는데, 이는 alias(알리아스)라고 발음되면서 컬럼명을 다른 이름으로
  - 지정하기 위해서 사용됩니다.
  -> 직접적으로 쓰기 보다는 기존에 있는 회원등급별할인액 table을 만들어서 적용하는 방식으로
    사용할 것이고, 회원테이블과 회원등급별할인액 테이블을 합치는 과정은 추후 수업 예정

  오늘 수업 이해가 안 될때
  참고 사이트 :
  https://www.youtube.com/watch?v=dgpBXNa9vJc&t=665s

- 그래서 현재까지 SQL 문을 직접 작성하여 DB 생성 및 테이블 생성하는방법
...
-- 어떻게 작성하면 될지를 확인할 필요가 있습니다. <- 데이터 가공
SELECT 회원아이디, 상품번호,
...
- 이상은 산술 연사자를 사용하여 데이터를 조회하는 방법에 해당합니다.
...
INSERT INTO person_list (person_id, person_name, person_age, person_email)
VALUES (100, '심민호', 25, 'zhf1066@naver,com');
...
-- 이름은 김이부터 김십까지
-- 나이는 21부터 29까지
-- 이메일은 b@test.com부터 -> j@test.com까지

-- 이후 select * from person_list;를 통해서 1 ~ 10 / 100번까지
-- 11개의 정보가 테이블에 들어가있는지 확인
INSERT INTO person_list (person_id, person_name, person_age, person_email)
...
SELECT person_id AS '학생id', person_name AS '학생 이름', person_age AS '학생 나이'
FROM person_list
WHERE person_id > 3 AND person_id <8;

-- 조회
-- 학생 이름 / 학생 나이 / 학생 이메일 컬럼 출력
-- 학생 id가 5이하이거나 7이상인 경우
-- 학생 나이 내림차순으로 정렬
SELECT person_name AS '학생 이름', person_age AS '학생 나이', person_email AS '학생 이메일'
FROM person_list
...
sql_class 테이블에 class_id = 1 class_member = 이일 class_grade = A

class_id = 2 class_member = 이이 class_grade = B

class_id = 3 class_member = 이삼 class_grade = C
...
- 동일한 방식으로 row를 생성하니까 굳이 insert into가 반복되지 않아도 된다
